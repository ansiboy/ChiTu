...
(function (chitu) {
    ...
    var PageNode = (function () {
        function PageNode(node) {
            ...
            this.loadingNode.innerHTML = '<div class="loading-form"><i class="icon-spinner icon-3x icon-spin"></i></div>';
    ...
    var Page = (function () {
        function Page(context, container) {
            ...
            this.pageNode = new PageNode(element);
        ...
        Page.prototype.hidePageNode = function () {
            if (this.swipe) {
        ...
        Page.prototype.showPageNode = function (swipe) {
            if (swipe === void 0) { swipe = undefined; }
            if (swipe) {
                var container_width = $(this.container).width();
                ...
                window.setTimeout(function () {
                    window['move'](self.node()).set('left', '0px').duration(times).end();
                }, 10);
                //====================================================
                window.setTimeout(function () {
                    self.on_shown({});
        ...
        Page.prototype.showBodyNode = function () {
            $(this.pageNode.node).show();
            $(this.pageNode.loadingNode).hide();
            $(this.pageNode.bodyNode).show();
        };
        Page.prototype.showLoadingNode = function () {
            $(this.pageNode.node).show();
            $(this.pageNode.bodyNode).hide();
            $(this.pageNode.loadingNode).show();
        };
        ...
        Page.prototype._loadViewModel = function () {
            ...
            this._loadViewModelResult = this._viewDeferred.pipe(function (html) {
                ...
                $(page.node()).find('.' + PAGE_BODY_CLASS_NAME).html(html);
        ...
        Page.prototype.open = function (args, swipe) {
            ...
            this.status = PageStatus.open;
            var self = this;
            self.on_showing(args);
            swipe = true;
            if (swipe)
                self.showPageNode(swipe);
            self.showLoadingNode();
            this._openResult = self._loadViewModel().pipe(function () {
        ...
        Page.prototype.close = function (args) {
            ...
            if (this._openResult != null) {
                this._openResult.reject({ message: 'close' });
            }
            var self = this;
            self.hidePageNode();
            self.on_closed(args);
            this.status = PageStatus.closed;
        };
        return Page;
    })();
    chitu.Page = Page;
...
(function (chitu) {
    ...
    var Application = (function () {
        ...
        Application.prototype.run = function () {
            ...
            var hashchange = function (event) {
                ...
                if (window.location['skip'] == null || window.location['skip'] == false)
                    app.showPage(hash.substr(1), args);
        ...
        Application.prototype.showPage = function (url, args) {
            ...
            var page = pages[name];
            if (page == null) {
                page = this._createPage(url, container);
                pages[name] = page;
            }
            this.previousPage = this._currentPage;
            this._currentPage = page;
            if (this.previousPage)
                this.previousPage.close();
            $.extend(args, routeData.values());
            var self = this;
            var result = $.Deferred();
            return page.open(args)
